<script>
	...
	import { devuimode, pageWidth, EventTable2, EventSize } from '$lib/store.mjs';
	...
	$: if ($EventTable2 == 1) {
		console.log('그룹 엑셀 저장 클릭');
		exportExcel(table1);
		$EventTable2 = 0;
	}

	$: if ($EventTable2 == 2) {
		console.log('그룹 PDF 저장/인쇄 클릭');
		exportPDF(table1);
		$EventTable2 = 0;
	}

	$: if ($EventTable2 == 3) {
		console.log('그룹 데이터 복사 클릭');
		exportClib(table1);
		$EventTable2 = 0;
	}
	...
	function GreClick() {
		let mname = new Array();
		if (stats.checked1 == false && stats.checked2 == false) {
			mname.push(-1);
		} else if (stats.checked1 == true && stats.checked2 == true) {
			mname.push(-1);
		} else {
...
<div class="EntireDiv {$EventSize == 1 ? '_big' : ''}">
	<div class="GrhEvt">
		...
		<div class="GvSrh">
			...
			<div />
			<div class="Check1">
				<Input type="checkbox1" bind:checked={stats.checked1}>모드변경</Input>
				<Input type="checkbox1" bind:checked={stats.checked2}>계획변경</Input>
			</div>
			<div />
			<Button bind:this={elems.btn} colored="blue" icon="search" on:click={GreClick}>조회</Button>
		</div>
	</div>
	<div />
	<div class="TableGvt">
...
<style>
	...
	.GrSrh {
		display: grid;
		grid-template-columns: 10% 10px 21% 10px 16% 15px 16% calc(37% - 35px);
	}
	...
	.TableGvt :global(.handsontable td) {
		color: #777777;
	}

	.TableGvt :global(.handsontable tr td:first-child) {
		font-size: 1.2rem;
	}
