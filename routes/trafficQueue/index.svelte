<script>
	...
	import { EventTable1 } from '$lib/store.mjs';
        ...
        // Option
        let option1 = {
            colWidths: [80,30,60,40,40,40,40,40,40],
            rowHeights: 23,
            renderer: 'html',
            renderAllRows: true,
            fixedRowsTop: 2,
        ...
        function makeData(res){
            ...
            if(res) {
                ...
                if(length){
                    ...
                    res.forEach((v, i) => {
                        let dirname = v.dir == 10 ? '북' : v.dir == 20 ? '동' : v.dir == 30 ? '남' : v.dir == 40 ? '서' : v.dir == 50 ? '북동' : v.dir == 60 ? '남동' : v.dir == 70 ? '남서' : '북서'
                        let arr = [document.lcCon.LCs[stats.lcno].lcname, dirname, v.cre_date.replace('T', ' '), v.vlen1, v.vlen2, v.vlen3, v.vlen4, v.vlen5, v.vlen6]
        ...
        function makeOption(res){
            ...
            if(res){ // 검색
                if(res.length < 40){
                    optionM.push({row: 42, col: 0, rowspan: 1, colspan: 3})
                } else {
                    optionM.push({row: res.length+2, col: 0, rowspan: 1, colspan: 3})
                }
            } else { // 초기화
                optionM.push({row: 42, col: 0, rowspan: 1, colspan: 3})
            }
        ...
        function searchData(e){
            ...
            ajax(`/s_length_${param[selected]}?starttime=${starttime}&endtime=${endtime}&lcno=${stats.lcno}`).then((res)=>{
        ...
        function exportExcel(table) {
            ...
            exportPlugin.downloadFile('csv', {
                ...
                fileExtension: 'csv',
                filename: `${stats.lcno}번_교차로_대기행렬(${startYear}.${startHour}시${startMin}분_${endYear}.${endHour}시${endMin}분)`,
                mimeType: 'text/csv',
                rowDelimiter: '\r\n',
                rowHeaders: false
            });
