...
<script>
    ...
    */
	...
	import Lti from './lti.svelte';
	import Lts from './lts.svelte';
	import Tst from './tfdtSt.svelte';
	import Tfs from './tfdtStatic.svelte';
	import Tfh from './tfdthis.svelte';
	...
	export const onSize = function (width, height) {
		if (width > 1200 || width <= 963) {
			...
			$TrfSize = 1;
			console.log('교통정보 사이즈 업');
		} else {
			stats.responsive = 0;
			$TrfSize = 0;
			console.log('교통정보 사이즈 다운');
		}
	...
	let items = [
		{ label: '교차로 교통정보', value: 1, component: Lti, tables: null, render: 0 },
		{ label: '교차로 교통통계', value: 2, component: Lts, tables: null, render: 0 },
		{ label: '검지기 현황', value: 3, component: Tst, tables: null, render: 0 },
		{ label: '검지기 이력', value: 4, component: Tfh, tables: null, render: 0 },
		{ label: '검지기 통계', value: 5, component: Tfs, tables: null, render: 0 }
	];
