<script>
		...
		ajax(`/ntcs_m_outped/${stats.lcno}`).then((res1) => {
			...
			ajax(`/ntcs_m_lsu/get-by-lcno?lcNo=${stats.lcno}`).then((res2) => {
				...
				// 요청 성공
					...
					table2.loadData(makeData(res2, 'lsu'));
					table2.updateSettings(makeOption('lsu'));
					if (!_.isEmpty(res2)) {
						isData[1] = 1;
					}
					isSuccess[1] = 1;
	...
	function upDownData(e) {
		document.createWindow('dbUpDown', false, e, { tabValue: 6, stats: stats });
	}

	// DB저장
	function saveData() {
		// dir
		let temp1 = table1.getSourceData(1, 4, 17, 23);
		...
		// lsu
		let temp2 = table2.getSourceData(1, 1, 1, 16);
...
<style>
	...
	/* DIR */
	.tabPage .tabContent > :global(div.dir > fieldset) {
		height: 392px;
		padding: 0;
	}

	.tabPage .tabContent :global(div.dir > fieldset > div) {
		display: grid;
	...
	/* LSU */
	.tabPage .tabContent :global(div.lsu > fieldset) {
		height: 47px;
		padding: 0;
	}

	.tabPage .tabContent :global(div.lsu > fieldset > div) {
		display: grid;
		grid-template-columns: 100%;
		height: 47px;
		overflow: hidden;
	...
	/* Headers */
	/* 방향별 출력맵 */
	.tabPage .tabContent :global(div.dir .handsontable tr:first-child td),
	.tabPage .tabContent :global(div.dir .handsontable tr td:nth-child(-n + 4)) {
		color: #999999;
		background: #f4f7f9;
	}

	/* LSU별 신호등 설정 */
	.tabPage .tabContent :global(div.lsu .handsontable tr td:nth-child(1)),
	.tabPage .tabContent :global(div.lsu .handsontable tr:nth-child(1) td) {
		color: #999999;
		background: #f4f7f9;
	}

	/* 방향별 출력맵 Text Overflow */
	.tabPage .tabContent :global(div.dir .handsontable tr:nth-child(5) td) {
		text-overflow: ellipsis;
		overflow: hidden;
		white-space: nowrap;
	}

	.tabPage .tabContent :global(div.dir .handsontable td) {
		font-size: 1.2rem;
	}
</style>
